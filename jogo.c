...
void inicializa_jogo(Jogo * jogo, int largura, int altura) {
	...
	inicializa_menu_inicial( jogo -> menu_principal, jogo -> fonte, jogo -> largura, jogo -> altura);
	inicializa_menu_pausa( jogo -> menu_opcoes, jogo -> fonte, jogo -> largura, jogo -> altura);
	inicializa_menu_opcoes( jogo -> menu_pausa, jogo -> fonte, jogo -> largura, jogo -> altura);
		...
		case MENU_INICIAL:
			if (ev.type == ALLEGRO_EVENT_KEY_DOWN)
				...
				case ALLEGRO_KEY_DOWN:
					// if (jogo->menu->botao_highlight < jogo->menu->numero_botoes - 1) {
					// 	jogo->menu->botoes[jogo->menu->botao_highlight].botao_cor = NORMAL;
					// 	jogo->menu->botao_highlight = (MYBUTTONS)(jogo->menu->botao_highlight + 1);
					// 	jogo->menu->botoes[jogo->menu->botao_highlight].botao_cor = HIGHLIGHT;
					// } else {
					// 	jogo->menu->botoes[jogo->menu->botao_highlight].botao_cor = NORMAL;
					// 	jogo->menu->botao_highlight = (MYBUTTONS) 0;
					// 	jogo->menu->botoes[jogo->menu->botao_highlight].botao_cor = HIGHLIGHT;
					// }
					break;
				case ALLEGRO_KEY_UP:
...
void cria_escudos(Jogo * jogo) {
	for (int i = 0, x = (jogo -> largura / N_ESCUDOS - TAMANHO_ESCUDO) / 2; i < N_ESCUDOS; i++, x += jogo -> largura / N_ESCUDOS) {
